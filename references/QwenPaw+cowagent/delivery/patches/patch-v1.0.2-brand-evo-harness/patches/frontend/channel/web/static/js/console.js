/* =====================================================================
   Evo-Harness Console - Main Application Script
   ===================================================================== */
...
function applyI18n() {
    ...
    if (docsLink) docsLink.removeAttribute('href');
}
...
const BRAND_NAME = 'Evo-Harness';
const BRAND_VERSION = 'v1.0.0';
let appConfig = { use_agent: false, title: BRAND_NAME, subtitle: '', providers: {}, api_bases: {} };
...
function startSSE(requestId, loadingEl, timestamp, titleInfo, replayItems) {
    ...
    function ensureBotEl() {
        ...
        botEl.innerHTML = `
            <img src="assets/logo.jpg" alt="Evo-Harness" class="w-8 h-8 rounded-lg flex-shrink-0">
...
function newChat(optimistic = true) {
    ...
    ws.innerHTML = `
        <img src="assets/logo.jpg" alt="Evo-Harness" class="w-16 h-16 rounded-2xl mb-6 shadow-lg shadow-primary-500/20">
        <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-100 mb-3">${appConfig.title || BRAND_NAME}</h1>
...
function initApp() {
    ...
    fetch('/api/version').then(r => r.json()).then(data => {
        ...
        document.getElementById('sidebar-version').textContent = `${BRAND_NAME} ${BRAND_VERSION}`;
    }).catch(() => {
        document.getElementById('sidebar-version').textContent = BRAND_NAME;
    });
    chatInput.focus();
}
