...
PATCH_DIR_NAME="patch-v1.0.7-browser-container-drissionpage"
PACKAGE_NAME="cowagent-patch-v1.0.7-browser-container-drissionpage.tar.gz"
...
main() {
  ...
  if tar -tzf "${DIST_DIR}/${PACKAGE_NAME}" \
    | grep -E '(^|/)compose/\.env$|support-bundles|__pycache__|\.pyc$|\.priv$|\.key$|id_(rsa|ed25519)|(^|/)storage/|(^|/)reference/' >/dev/null; then
  ...
  if rg -l 'BEGIN (OPENSSH|RSA|EC|DSA) PRIVATE KEY|SSH_SERVER_[A-Z0-9_]+_PASSWORD=[^<[:space:]]' \
    "${PATCH_ROOT}/patches" "${PATCH_ROOT}/scripts" >/dev/null; then
    die "package source contains a private key or plaintext password"
  fi
