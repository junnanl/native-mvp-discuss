...
PATCH_DIR_NAME="patch-v1.0.4-officecli-toolpack"
PACKAGE_NAME="cowagent-patch-v1.0.4-officecli-toolpack.tar.gz"
...
main() {
  ...
  if tar -tzf "${DIST_DIR}/${PACKAGE_NAME}" \
    | grep -E '(^|/)compose/\.env$|support-bundles|__pycache__|\.pyc$|\.pem$|\.key$|(^|/)storage/' >/dev/null; then
    die "package contains forbidden runtime, secret, cache, or storage path"
  fi
