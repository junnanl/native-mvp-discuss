...
PATCH_VERSION="1.0.4-officecli-toolpack"
REQUIRED_PATCH_VERSION="1.0.3-runtime-config-persistence"
...
image_for_target() {
  read_env_value COW_IMAGE || echo "cowagent-offline:2.1.7-base.1-dev"
}
...
normalize_mount_permissions() {
  ...
  docker run --rm \
    ...
    -c '
      set -e
      mkdir -p /mnt/root/storage/cow/skills/officecli /mnt/root/storage/cow/patch-state /mnt/root/toolpacks/binaries/officecli
      chmod a+rwx /mnt/root/storage /mnt/root/storage/cow /mnt/root/storage/cow/skills /mnt/root/toolpacks /mnt/root/toolpacks/binaries
      chmod -R a+rwx /mnt/root/storage/cow/skills/officecli /mnt/root/storage/cow/patch-state /mnt/root/toolpacks/binaries/officecli
      [ ! -f /mnt/root/storage/cow/skills/skills_config.json ] || chmod a+rw /mnt/root/storage/cow/skills/skills_config.json
    '
}

register_officecli_skill() {
  local image
  image="$(image_for_target)"
  docker run --rm \
    -v "${TARGET_ROOT}:/mnt/root" \
