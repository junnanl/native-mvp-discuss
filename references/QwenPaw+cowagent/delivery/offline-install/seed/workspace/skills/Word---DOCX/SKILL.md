...
description: "Create, inspect, and edit complex Microsoft Word documents with reliable styles, numbering, tracked changes, tables, sections, and compatibility checks. Use OfficeCLI first for ordinary structural edits, validation, and render checks; use this Skill's OOXML workflow only when advanced fidelity requires it."
...
本 Skill 负责复杂 Word 文档的保真规则。普通创建、结构化读取、最小编辑、OpenXML 校验和截图渲染优先使用 `officecli` Skill 指定的固定本地二进制；每次修改后执行 `close` 和 `validate`。

当文件包含复杂修订、宏、受保护内容、特殊 OOXML 部件或 OfficeCLI 未覆盖的能力时，才使用本 Skill 后文的 OOXML 深度编辑方法。不得执行 `officecli install`、联网安装或覆盖原始附件；复杂编辑后的最终文件仍必须完成离线校验和渲染检查。
...
## Related Skills
离线交付环境禁止通过 ClawHub 安装或同步 Skill。以下仅为能力关联说明，不执行安装：
...
离线环境不执行 `clawhub star` 或 `clawhub sync`。
