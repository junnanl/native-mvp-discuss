...
import { Search, RotateCcw, Plus, FileText, Eye, Edit3, FilePlus } from 'lucide-react'
import { fundApplyList, getCostFundStatusText, getInvestFundStatusText, getFundStatusClass, investmentTypeMap } from '@/data/mock'
import type { FundApplyItem, FundType } from '@/data/mock'
...
export default function ICTInvestFundManage({ onNavigate }: FundApplyListProps) {
  const activeTab: ListTab = 'invest'
  const [tab, setTab] = useState<'ict' | 'gov'>('ict')
  ...
  const listTitle = tab === 'ict' ? 'ICT项目投资立项工单列表' : '政企项目投资立项工单列表'
  ...
  const handleCreateInvest = () => {
    onNavigate?.(tab === 'ict' ? '/finance/fund/invest-apply' : '/finance/fund/gov-invest-apply')
  }

  const handleDetail = (item: FundApplyItem) => {
    if (tab === 'gov') {
      onNavigate?.(`/finance/fund/gov-invest-detail/${item.id}`)
    } else {
      onNavigate?.(`/finance/fund/detail/${item.id}`)
    }
  }

  const handleEdit = (item: FundApplyItem) => {
    if (item.type === 'cost') {
      onNavigate?.(`/finance/fund/cost-apply/${item.id}`)
    } else if (tab === 'gov') {
      onNavigate?.(`/finance/fund/gov-invest-apply/${item.id}`)
    } else {
      onNavigate?.(`/finance/fund/invest-apply/${item.id}`)
    }
  }

  const canEdit = (item: FundApplyItem) => {
    if (item.type === 'cost') return item.status === '01'
          ...
          <div className="flex border-b border-gray-100">
            <button
              ...
              className={`px-6 py-3 text-sm font-medium transition-colors relative ${
                tab === 'ict' ? 'text-[#1677FF]' : 'text-gray-600 hover:text-gray-800'
              }`}
            >
              ICT项目投资立项工单
              {tab === 'ict' && (
            ...
            <button
              ...
              onClick={() => setTab('gov')}
              className={`px-6 py-3 text-sm font-medium transition-colors relative ${
                tab === 'gov' ? 'text-[#1677FF]' : 'text-gray-600 hover:text-gray-800'
              }`}
            >
              政企项目投资立项工单
              {tab === 'gov' && (
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
          {tab === 'ict' ? (
            <div className="grid grid-cols-3 gap-x-6 gap-y-3">
              ...
              <div className="flex items-center gap-2">
                <label className="w-28 text-right text-sm text-gray-700 shrink-0">工单状态</label>
                <select
                  value={filter.status}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {pagedList.length === 0 ? (
                  <tr>
                    <td colSpan={tab === 'ict' ? (activeTab === 'invest' ? 10 : 9) : (activeTab === 'invest' ? 8 : 7)} className="px-4 py-12 text-center text-gray-400">
                      暂无数据
                    </td>
                  </tr>
                ) : (
