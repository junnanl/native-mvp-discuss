...
import { ArrowLeft, RotateCcw, Send, X, HelpCircle, ChevronDown, ChevronRight } from 'lucide-react'

interface ExpenseBillApprovalProps {
...
// 主页面
// ============================================================
export default function ExpenseBillApproval({ onNavigate }: ExpenseBillApprovalProps) {
  // 报账单信息状态（只读，数据补充完整）
  ...
  const [summary] = useState('2026年6月IDC数据中心项目设备采购及软件开发支出报账，本期报账金额合计35.17万元，付款对象为项目供应商。')
  const [remark] = useState('本次报账支付金额与合同约定付款方式不一致，需人工核验。')
      ...
      <div className="w-full min-w-0 mx-auto p-3 space-y-3">
        ...
        <div className="bg-white rounded-lg shadow-sm px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-gray-800">项目类费用报账单审批</h2>
          </div>
        </div>

        {/* 1. 报账单信息（复制自报账详情页面组件，独立维护） */}
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {paymentDetailRows.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-3 py-8 text-center text-gray-400">暂无数据</td>
                  </tr>
                ) : (
