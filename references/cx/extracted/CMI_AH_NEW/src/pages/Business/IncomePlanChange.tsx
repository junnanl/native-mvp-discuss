...
interface IncomePlanChangeProps {
...
export default function IncomePlanChange({ onNavigate, contractId }: IncomePlanChangeProps) {
  ...
  const [changeRemark, setChangeRemark] = useState('因客户业务需求调整，原合同中IT收入计划的产品目录及金额需进行变更。其中维保费金额由原50万元调整为60万元，设备费由原200万元调整为220万元，同时新增一项平台使用费收入项。CT收入计划中宽带产品订购数量由50调整为80，相应概算收入金额同步调整。变更后总收入金额增加约30万元，已附相关证明材料，请审批。')
  const [proofFiles, setProofFiles] = useState<{ id: string; name: string; size: string; uploadTime: string }[]>([
    { id: 'proof-1', name: '合肥市工商银行智能监控系统实施变更证明.doc', size: '2.5 MB', uploadTime: '2026-07-15 10:30:00' }
  ])
  ...
  const handleSubmit = () => {
    ...
    if (processResult === 'transferred' && !flowApprover) {
      alert('请选择合同解析人员')
      return
    }
    if (processResult === 'completed' && !flowApprover) {
      alert('请选择解决方案经理')
      return
    }
    modal.confirm('确定提交吗？', '提交').then(ok => {
      if (ok) {
        alert('提交成功')
        onNavigate?.(`/finance/contract/income-plan-change-confirm/${contractId || 'CT2026060001'}`)
      }
    })
  }
      ...
      <div className="w-full min-w-0 mx-auto p-3 space-y-3">
        ...
        <div className="bg-white rounded-lg shadow-sm px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-gray-800">收入计划调整</h2>
          </div>
        </div>
                  ...
                  {proofFiles.map(attach => (
                    ...
                    >
                      ...
                      <div className="flex items-center flex-1 min-w-0 ml-3">
                        <span className="text-sm text-gray-800 truncate">{attach.name}</span>
                        <span className="inline-flex items-center px-1.5 py-0.5 text-xs text-gray-600 bg-gray-100 rounded shrink-0 ml-3">证明材料</span>
                      </div>
                      <span className="text-xs text-gray-400 shrink-0 ml-2">{attach.size}</span>
                      <span className="text-xs text-gray-400 shrink-0 ml-2">{attach.uploadTime}</span>
                      <button
                        type="button"
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <CommonSearchableSelect
                            value={flowApprover}
                            onChange={setFlowApprover}
                            options={solutionManagerOptions}
          ...
          >
            <div className="grid grid-cols-2 gap-x-5 gap-y-4">
              ...
              <FormRow label="计划订购金额" required>
                {ctIncomeModalState.editingRow ? (
                  <div className="w-full px-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-md text-gray-700">
                    {ctIncomeModalState.editingRow.plannedTariffAmount || '-'}
                  </div>
                ) : (
                  <NumberInput
                    value={''}
