...
interface PrepaymentManagementProps {
...
const prepaymentProjectCodes = ['PRJ-2026-HF-001', 'PRJ-2026-WH-002', 'PRJ-2026-BB-003', 'PRJ-2026-HF-004', 'PRJ-2026-AH-005']

const mockPrepayments: PrepaymentItem[] = Array.from({ length: 25 }).map((_, i) => {
  ...
  return {
    id: `prepay-${i + 1}`,
    code: `YF${2026}${String((i % 12) + 1).padStart(2, '0')}${String(i + 1).padStart(5, '0')}`,
    erpCode: `ERP${2026}${String((i % 12) + 1).padStart(2, '0')}${String(i + 1).padStart(5, '0')}`,
    projectCode: prepaymentProjectCodes[projectIdx],
    projectName: prepaymentProjectNames[projectIdx],
...
export default function PrepaymentManagement({ onNavigate }: PrepaymentManagementProps) {
  ...
  const pageSize = 10
  const [list, setList] = useState<PrepaymentItem[]>(mockPrepayments)
  ...
  const handleRefreshStatus = (item: PrepaymentItem) => {
  ...
  const handleDetail = (item: PrepaymentItem) => {
    onNavigate?.(`/finance/expense/prepayment/detail/${item.id}`)
  }

  // 修改：仅报账单状态为草稿时展示
  const canEdit = (item: PrepaymentItem) => {
    return item.status === 'draft'
  }

  const handleEdit = (item: PrepaymentItem) => {
    onNavigate?.(`/finance/expense/prepayment/edit/${item.id}`)
  }

  const canRefreshStatus = (item: PrepaymentItem) => {
    return item.status !== 'cancelled' && item.status !== 'completed'
  }

  const handleCreatePrepayment = () => {
    onNavigate?.('/finance/expense/prepayment/create')
  }
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="grid grid-cols-3 gap-x-6 gap-y-3">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {prepaymentStatusOptions.map(opt => (
