...
import { ArrowLeft, Check, RotateCcw, ChevronRight, ChevronDown, X, Plus, HelpCircle, Upload, FileText } from 'lucide-react'
...
function InfoField({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start min-h-[32px] py-0.5">
      <label className="w-44 text-right text-sm text-gray-700 shrink-0 pr-2 break-all">{label}：</label>
      <div className="flex-1 min-w-0 text-sm text-gray-700 break-all">{value || '-'}</div>
    </div>
  )
}

interface IncomeExpenseContractParseProps {
...
export default function IncomeExpenseContractParse({ onNavigate, contractId }: IncomeExpenseContractParseProps) {
  ...
  const handleSubmit = () => {
    ...
    modal.confirm('确定提交吗？', '提交').then(ok => {
      if (ok) {
        onNavigate?.(`/finance/contract/income-expense-parse-approval/${contractId || 'CT2026060002'}`)
      }
    })
  }
      ...
      <div className="w-full min-w-0 mx-auto p-3 space-y-3">
        ...
        <div className="bg-white rounded-lg shadow-sm px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-gray-800">有收有支合同解析</h2>
          </div>
        </div>
