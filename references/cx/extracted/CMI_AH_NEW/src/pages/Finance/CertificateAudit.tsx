...
interface CertificateAuditProps {
  onNavigate?: (path: string) => void
  contractId?: string
}

export default function CertificateAudit({ onNavigate, contractId }: CertificateAuditProps) {
  ...
  const [onlineAuditStaff, setOnlineAuditStaff] = useState('')
  ...
  const handleSubmit = () => {
    alert('三证一书稽核提交成功！')
    if (onNavigate) {
      onNavigate(`/finance/income/confirm/approval/${contractId}`)
    }
  }
      ...
      <div className="w-full min-w-0 mx-auto p-3 space-y-3">
        <div className="bg-white rounded-lg shadow-sm px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-gray-800">三证一书稽核</h2>
          </div>
        </div>
        ...
        <ApprovalInfo onResultChange={setApprovalResult} showTransferButton />
                ...
                <div className="flex items-center min-h-[36px]">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="px-3 py-2 text-sm bg-gray-50 border border-gray-200 rounded-md text-gray-700">
                      {approvalResult === 'approved' && '收入计划确认审批'}
                      {approvalResult === 'rejected' && '收入计划确认发起'}
                      {approvalResult === 'transferred' && '三证一书稽核'}
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <div className="flex items-center min-h-[36px]">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <SearchableSelect
                            value={onlineAuditStaff}
                            onChange={setOnlineAuditStaff}
                            options={approverOptions}
                            placeholder="请选择下一步处理人"
                          />
                        </div>
