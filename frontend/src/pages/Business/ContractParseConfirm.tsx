...
// 主页面
// ============================================================
interface ContractParseConfirmProps {
...
export default function ContractParseConfirm({ onNavigate, contractId, readOnly = false }: ContractParseConfirmProps) {
  ...
  const openCtIncomeModal = (row?: CTIncomeRow, productName?: string) => {
    if (row) {
      const display = row.productFullName && row.productCode ? `【${row.productCode}】${row.productFullName}` : ''
      setCtProductDisplay(display)
  ...
  const handleCtProductNameChange = (v: string) => {
    ...
    const onlyOneTariff = tariffOptions.length === 1 ? tariffOptions[0] : ''
    setCtIncomeModalState(prev => ({
      ...prev,
      editingRow: prev.editingRow ? {
        ...prev.editingRow,
        ...
        tariffName: onlyOneTariff,
        taxRate: onlyOneTariff ? ctTariffTaxMap[onlyOneTariff] || '' : '',
        mgmtProductName: onlyOneTariff ? ctTariffMgmtNameMap[onlyOneTariff] || '' : '',
        coaSubject: onlyOneTariff ? ctTariffCoaMap[onlyOneTariff] || prev.editingRow.coaSubject : ''
      } : null
    }))
    ...
    modal.confirm('确定提交吗？', '提交').then(ok => {
      if (ok) {
        ...
        if (approvalResult === 'approved') {
          onNavigate?.(`/finance/contract/parse-approval/${contractId || 'CT2026060001'}`)
        } else {
      ...
      <div className="w-full min-w-0 mx-auto p-3 space-y-3">
        ...
        <div className="bg-white rounded-lg shadow-sm px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-gray-800">前向合同解析确认与补充</h2>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm">
          ...
          {trailExpanded && (
            <div className="p-4">
              <div className="mb-4 px-3 py-2 text-xs text-orange-700 bg-orange-50 border border-orange-200 rounded-md flex items-start gap-2">
