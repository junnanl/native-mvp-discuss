...
interface IncomePlanChangeInitProps {
...
export default function IncomePlanChangeInit({ onNavigate, contractId }: IncomePlanChangeInitProps) {
  ...
  const handleSubmit = () => {
    ...
    modal.confirm('确定提交吗？', '提交').then(ok => {
      if (ok) {
        alert('提交成功')
        onNavigate?.(`/finance/contract/income-plan-change/${contractId || 'CT2026060001'}`)
      }
    })
  }
      ...
      <div className="w-full min-w-0 mx-auto p-3 space-y-3">
        ...
        <div className="bg-white rounded-lg shadow-sm px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-gray-800">收入计划调整发起</h2>
          </div>
        </div>
