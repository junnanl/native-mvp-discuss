...
const mockPurchaseOrders: PurchaseOrderItem[] = Array.from({ length: 15 }).map((_, i) => ({
  id: `PO-${String(i + 1).padStart(6, '0')}`,
  orderCode: `PO-2026-${String(i + 1).padStart(6, '0')}`,
  ...
  contractName: ['智慧医疗系统采购合同', '数字政府平台建设合同', '智慧教育云服务合同', '智慧交通集成合同', '智慧城市总包合同'][i % 5],
  purchaseRequestCode: `PR-2026-${String(i + 1).padStart(6, '0')}`,
  documentType: ['设备采购', '服务采购', '软件开发', '系统集成', '物资采购'][i % 5],
  amountExcludingTax: ((i + 1) * 85000).toFixed(2),
  amountIncludingTax: ((i + 1) * 85000 * 1.13).toFixed(2),
  status: ['pending-parse', 'pending-approval', 'approving', 'approval-rejected', 'approval-passed'][i % 5],
  createTime: `2026-06-${String((i % 28) + 1).padStart(2, '0')} ${String((i * 3) % 24).padStart(2, '0')}:${String((i * 7) % 60).padStart(2, '0')}:00`,
  creator: ['张三', '李四', '王五', '赵六', '孙七'][i % 5],
  applicant: ['周八', '吴九', '郑十', '冯十一', '陈十二'][i % 5],
  supplierCode: `SUP-${String(i + 1).padStart(4, '0')}`,
  supplierName: ['合肥科技有限公司', '芜湖信息技术公司', '蚌埠智能系统公司', '合肥网络科技公司', '安徽软件技术公司'][i % 5]
}))
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="grid grid-cols-2 gap-x-6 gap-y-3">
            <div className="flex items-center gap-2">
              <label className="w-28 text-right text-sm text-gray-700 shrink-0">采购订单编号</label>
              <input
                type="text"
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-gray-800">采购订单列表</h3>
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
                <tr className="text-gray-500">
                  <th className="px-3 py-2.5 text-left font-medium whitespace-nowrap">采购订单编号</th>
                ...
                {pagedOrders.length === 0 ? (
                  ...
                  pagedOrders.map(item => (
                    <tr key={item.id} className="hover:bg-gray-50/50">
                      ...
                      <td className="px-3 py-3 text-gray-600 whitespace-nowrap">{item.purchaseRequestCode}</td>
                      <td className="px-3 py-3 text-gray-600 whitespace-nowrap">{item.documentType}</td>
                      ...
                      <td className="px-3 py-3 text-gray-600 whitespace-nowrap">{item.applicant}</td>
                      <td className="px-3 py-3 text-gray-600 whitespace-nowrap">{item.supplierCode}</td>
                      <td className="px-3 py-3 text-gray-600 whitespace-nowrap">{item.supplierName}</td>
                      <td className="px-3 py-3 whitespace-nowrap sticky right-0 bg-white hover:bg-gray-50 shadow-[-4px_0_4px_-2px_rgba(0,0,0,0.05)]">
                        <div className="flex items-center gap-3">
                          ...
                          {item.status === 'approval-passed' && (
                            <button className="text-[#1677FF] hover:underline text-xs">计划变更</button>
                          )}
                          {item.status === 'pending-parse' && (
                            <button
                              className="text-[#1677FF] hover:underline text-xs"
                              onClick={() => onNavigate?.(`/finance/contract/order/purchase/parse/${item.id}`)}
                            >
                              订单解析
                            </button>
                          )}
