...
import { Search, RotateCcw, RefreshCw, FileText, Plus, FilePlus, Edit, Copy } from 'lucide-react'
...
const projectCodes = ['PRJ-2026-HF-001', 'PRJ-2026-WH-002', 'PRJ-2026-BB-003', 'PRJ-2026-HF-004', 'PRJ-2026-AH-005']

const mockExpenses: ExpenseItem[] = Array.from({ length: 25 }).map((_, i) => {
  ...
  return {
    id: `exp-${i + 1}`,
    code: `BZ${2026}${String((i % 12) + 1).padStart(2, '0')}${String(i + 1).padStart(5, '0')}`,
    erpCode: `ERP${2026}${String((i % 12) + 1).padStart(2, '0')}${String(i + 1).padStart(5, '0')}`,
    projectCode: projectCodes[projectIdx],
...
export default function ExpenseReimbursement({ onNavigate }: ExpenseReimbursementProps) {
  ...
  const pageSize = 10
  const [list, setList] = useState<ExpenseItem[]>(mockExpenses)
  ...
  const handleRefreshStatus = (item: ExpenseItem) => {
  ...
  const handleDetail = (item: ExpenseItem) => {
    if (item.withContract) {
      onNavigate?.(`/finance/expense/detail-with-contract/${item.id}`)
    } else {
      onNavigate?.(`/finance/expense/detail-without-contract/${item.id}`)
    }
  }

  const canRefreshStatus = (item: ExpenseItem) => {
    return item.status !== 'cancelled' && item.status !== 'completed'
  }

  // 修改：仅报账单状态为草稿时展示
  const canEdit = (item: ExpenseItem) => {
    return item.status === 'draft'
  }

  const handleEdit = (item: ExpenseItem) => {
    onNavigate?.(`/finance/expense/edit/${item.id}`)
  }

  // 复制：报账单状态不为草稿时展示
  const canCopy = (item: ExpenseItem) => {
    return item.status !== 'draft'
  }

  const handleCopy = (item: ExpenseItem) => {
    onNavigate?.(`/finance/expense/copy/${item.id}`)
  }

  const handleCreateWithContract = () => {
    onNavigate?.('/finance/expense/create-with-contract')
  }

  const handleCreateWithoutContract = () => {
    onNavigate?.('/finance/expense/create-without-contract')
  }

  const handleSupplement = () => {
    onNavigate?.('/finance/expense/supplement')
  }
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-gray-800">报账单列表</h3>
              <span className="text-xs text-gray-400">共 {filteredList.length} 条</span>
            </div>
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={handleCreateWithContract}
              ...
              <button
                type="button"
                onClick={handleCreateWithoutContract}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs text-white bg-[#1677FF] rounded-md hover:bg-[#1668DD] transition-colors"
              >
                <Plus className="w-3.5 h-3.5" />
                发起报账（网格小微项目）
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs text-blue-600 bg-white border border-blue-600 rounded-md hover:bg-blue-50 transition-colors"
              >
