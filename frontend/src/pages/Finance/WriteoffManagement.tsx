...
import { Search, RotateCcw, RefreshCw, Edit, FileText, Plus } from 'lucide-react'
...
const writeoffProjectCodes = ['PRJ-2026-HF-001', 'PRJ-2026-WH-002', 'PRJ-2026-BB-003', 'PRJ-2026-HF-004', 'PRJ-2026-AH-005']

const mockWriteoffs: WriteoffItem[] = Array.from({ length: 23 }).map((_, i) => {
  ...
  return {
    id: `wroff-${i + 1}`,
    uniqueId: `UID-WOFF-${String(i + 1).padStart(8, '0')}`,
    code: `CX${2026}${String((i % 12) + 1).padStart(2, '0')}${String(i + 1).padStart(4, '0')}`,
    erpCode: `ERP${2026}${String((i % 12) + 1).padStart(2, '0')}${String(i + 1).padStart(5, '0')}`,
    projectCode: writeoffProjectCodes[projectIdx],
    projectName: writeoffProjectNames[projectIdx],
    ...
    writeoffMonth: `2026-${String((i % 12) + 1).padStart(2, '0')}`
  }
})
...
export default function WriteoffManagement({ onNavigate }: WriteoffManagementProps) {
  ...
  const pageSize = 10
  const [list, setList] = useState<WriteoffItem[]>(mockWriteoffs)
  ...
  const handleRefreshStatus = (item: WriteoffItem) => {
  ...
  const handleDetail = (item: WriteoffItem) => {
    onNavigate && onNavigate(`/finance/expense/writeoff/detail/${item.id}`)
  }

  const handleEdit = (item: WriteoffItem) => {
    alert(`修改冲销单：${item.code}`)
  }

  // 是否显示"刷新状态"按钮：冲销单状态不为已作废、已完成
  const canRefreshStatus = (item: WriteoffItem) => {
    return item.status !== 'cancelled' && item.status !== 'completed'
  }

  // 是否显示"修改"按钮：省内审批状态为审批不通过 或 冲销单状态为已作废
  const canEdit = (item: WriteoffItem) => {
    return item.approvalStatus === 'rejected' || item.status === 'cancelled'
  }
        ...
        <div className="bg-white rounded-lg shadow-sm p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-gray-800">冲销单列表</h3>
              <span className="text-xs text-gray-400">共 {filteredList.length} 条</span>
            </div>
            <div className="flex items-center gap-4">
              <button
                type="button"
                onClick={() => onNavigate && onNavigate('/finance/expense/writeoff/create')}
